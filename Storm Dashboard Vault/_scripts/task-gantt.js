...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   VERBATIM from _scripts/goal-panel.js — those rules are pinned by tests in
   the Storm and work vaults and must stay byte-identical. The `goal-panel.txt`
   line references in the section headers below are rx4's original wording,
   kept byte-for-byte so the rx4 lineage pin still holds (the file is .js now).
...
/* ── 15b. PAUSE §4 — pause / resume this one task. Line-based frontmatter
      (setFm ported VERBATIM from the work vault's initHub.js — processFrontMatter
